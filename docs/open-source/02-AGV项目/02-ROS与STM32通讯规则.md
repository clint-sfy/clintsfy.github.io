...
| 3      | 叉车工作模式           | enum   AGV_MODEL         |
| 4      | 预留位                 | -                        |
| 5-6    | X轴目标线速度          | mm/s                     |
| 7-8    | Y轴目标线速度          | mm/s  (仅全向底盘有效)   |
| 9-10   | Z轴目标角速度/舵轮角度 | rad/s × 1000 / rad       |
| 11-12  | *货叉*离地高度         | mm                       |
...
| 8-9     | 电池电压         | mV                                    |
| *10-11* | Fcode            |                                       |
| 12      | 托盘任务完成情况 | enum  PalletTaskStauts                |
| 13      | 行程开关是否触发 | bool    (true 触发     false没有触发) |
| 14      | 预留位           | -                                     |
| 15      | 预留位           | -                                     |
| 16      | 预留位           | -                                     |
| 17      | 预留位           | -                                     |
...
```cpp
typedef enum {
	AGV_CLEAR_MODEL,                     // 单片机必须给ROS发数
	AGV_USE_ROS_MODEL,				     // 只有在路径跟踪时才会启动 
	AGV_ELECTROMAGNETIC_MODEL,		     // 电磁导航模式
    AGV_CLOSE_MODEL,                     // 允许单片机不给ROS发数
}AGV_MODEL;
```
...
```cpp
typedef enum { {
    Pallet_NO_TASK,           // 无托盘任务
    Pallet_EXECUTING ,        // 执行任务中
    Pallet_LOWEST_COMPLETED,  // 最低位已完成
    Pallet_HIGHEST_COMPLETED  // 最高位已完成
}PalletTaskStauts;
```
