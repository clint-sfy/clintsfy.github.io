...
- 每篇必须按“专业术语→白话解释→简单案例→易混点→课后小问→本节小结→快速回顾”组织；课后小问保留问答形式，每题附简洁答案和解析，用于概念自测。
- 删除课程文章中的 Markdown/Unicode checkbox、实践任务和练习题；课后小问使用普通编号列表，不提供不可持久化的交互假象。
...
- 课后小问是否保留问答形式，每题都紧跟简洁答案和解析，用于概念自测而不是练习任务。
...
新增测试并断言：33 个 Markdown、32 篇课程文章、12 个章节；逐篇 frontmatter 存在 `title`、`description`、`category`、`tags`；正文包含“学习目标”“核心知识点”“简单案例”“易混点”“课后小问”“本节小结”“快速回顾”和至少一个非空 `java` 代码块；“课后小问”至少包含两道问题，且每道问题都紧跟“答案”和“解析”；课程目录不含 checkbox、`实践任务`、`练习题`或明显占位标记。失败信息必须包含相对文件路径和规则名。
...
Expected: 新测试因 28 篇提纲正文、缺少课后小问答案/解析、旧 checkbox/实践任务而失败；现有测试仍通过。保存失败文件清单作为后续批次输入。
...
保留已有解释、表格和案例，补齐“易混点”、带答案/解析的“课后小问”和“快速回顾”，删除复选框与任务式表达；避免重复讲解方法和重载。
...
逐篇确认术语、白话解释、案例结果、易混点和课后小问相互一致；课后小问有简洁答案/解析，没有大型示例、练习任务或重复堆砌。
...
删除“实践任务/练习”承诺，改为说明“专业术语、白话解释、简单案例、易混点、课后小问（含答案/解析）、本节小结和快速回顾”；保持阶段、链接和进阶出口不变。
...
由未参与正文编写的审阅 Agent 对照规范检查专业准确性、JDK 20 兼容、案例简洁度、易混点辨析、课后小问答案/解析质量和文件范围。Critical/Important 问题必须修复并重新验证。
...
请求课程入口与至少八个代表页面，确认 HTTP 200、标题/侧栏存在、“课后小问”及其答案/解析、“快速回顾”可见，旧 checkbox/实践任务不存在；将线上 URL、最终 commit 和部署状态写入报告。
