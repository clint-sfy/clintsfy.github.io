...
record 适合“数据是什么”比“生命周期怎么变”更重要的值载体，不是自动深复制或自动校验的魔法。组件引用在构造后不能换，但可变集合内容仍可改；需要深不可变时应复制并暴露不可变视图。`sealed` 把可扩展范围写进类型定义，使编译器知道已知子类型，适合表达有限的领域结果，但开放扩展的插件模型不应强行封闭。
...
解析：构造器应复制列表并保存 `List.copyOf(items)` 等不可变视图，才能把不变式扩展到集合内容。
